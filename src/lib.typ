...
#let zine-page(
  ...
  ///
  /// Internal margin for page content inside the frame.
  /// Default is 5mm on all sides so content has a comfortable inset
  /// from the frame edge; override by passing `margin: <length|map>`
  /// into `zine-page` if you need different spacing.
  ///
  /// -> length
  margin: 5mm,
...
/// construct an eight-page zine with custom frame size (45mm x 80mm)
/// 
/// This version uses fixed frame dimensions of 45mm x 80mm instead of
/// calculating from the printer page size.
///
...
#let zine8(
  ...
  // Fixed dimensions for each frame: 45mm x 80mm
  let zine-page-width = 45mm;
  let zine-page-height = 80mm;
  ...
  if digital {
    ...
    set page(height: zine-page-height, width: zine-page-width, margin: 0pt)
...
/// construct 16-page zine with custom frame size (45mm x 80mm)
/// 
/// This version uses fixed frame dimensions of 45mm x 80mm instead of
/// calculating from the printer page size.
///
